...
var filter = require("./filter"),
    parsePointer = require("./common").parsePointer,
...
function getCbFactory(type, matches) {
    switch (type) {
        case queryGet.ALL:
            return function cbGetPointer(obj, cb, pointer) {
                return function (key) {
                    matches.push([obj[key], key, obj, join(pointer, key)]);
                }
            }

        case queryGet.POINTER:
            return function cbGetValue(obj, cb, pointer) {
                return function (key) {
