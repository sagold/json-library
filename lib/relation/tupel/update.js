...
var updateByType = {
	...
	"has_many:through:alias": function (r, pk) {
		...
		o.forEach(relatedTupels, function updateChild(child) {
			...
			addTupelToRelatedModel(r.relatedModel, pkOfRelatedModel, child);
			addReferenceToRelatedModel(r.pivot, pk, pkOfRelatedModel);
		});
	},
	...
	"has_many:foreign_key:alias": function (r, pk) {
		...
		o.forEach(relatedTupels, function updateChild(child) {
			...
			var pkOfRelatedModel = getPrimaryKey(r.relatedModel, child, pk);
			addTupelToRelatedModel(r.relatedModel, pkOfRelatedModel, child);
			addReferenceToRelatedModel(tupel, r.foreign_key, pkOfRelatedModel);
		});
	},
	...
	"has_one:through:alias": function (r, pk) {
		...
		addTupelToRelatedModel(r.relatedModel, pkOfRelatedModel, relatedTupel);
		setReferenceToRelatedModel(r.pivot, pk, pkOfRelatedModel);
	},
	...
	"has_one:foreign_key:alias": function (r, pk) {
		...
		var pkOfRelatedModel = getPrimaryKey(r.relatedModel, relatedTupel, pk);
		addTupelToRelatedModel(r.relatedModel, pkOfRelatedModel, relatedTupel);
		setReferenceToRelatedModel(tupel, r.foreign_key, pkOfRelatedModel);
	},
...
/**
 * Update pivot tables based on the given parent tupel (owning relationship)
 *
 * @param  {[type]} data        [description]
 * @param  {[type]} defRelation [description]
 * @param  {[type]} tupelPk     [description]
 * @param  {[type]} replace     [description]
 * @return {[type]}             [description]
 */
function updatePivot(data, defRelation, tupelPk, replace) {
	var model,
